<?php
/**
 * Setup Approved Architecture Pages in WordPress DB
 */
...
$pages = array(
    array(
        'title'    => 'Empresa',
        'slug'     => 'empresa',
        'template' => 'page-empresa.php',
        'content'  => 'Información institucional de ROLBAG: historia, trayectoria desde 2006, taller de confección en Santiago Centro, clientes históricos, misión y visión.',
    ),
    array(
        'title'    => 'Soluciones',
        'slug'     => 'soluciones',
        'template' => 'page-soluciones.php',
        'content'  => 'Soluciones operativas de ROLBAG para Almacenes, Bodegajes y Logística en todo Chile.',
    ),
    array(
        'title'    => 'Contacto',
        'slug'     => 'contacto',
        'template' => 'page-contacto.php',
        'content'  => 'Canal oficial de contacto comercial, cotizaciones B2B, WhatsApp y ubicación del taller central en Santiago Centro.',
    ),
    array(
...
foreach ( $pages as $p ) {
    ...
    if ( $existing ) {
        ...
        WP_CLI::success( "Página actualizada: {$p['title']} (/{$p['slug']}) -> {$p['template']}" );
    } else {
        ...
        if ( ! is_wp_error( $page_id ) ) {
            ...
            WP_CLI::success( "Página creada: {$p['title']} (/{$p['slug']}) -> {$p['template']}" );
        }
    }
}

// Redirect 'nosotros' or 'soluciones-a-medida' if someone navigates there
$old_nosotros = get_page_by_path( 'nosotros' );
if ( $old_nosotros ) {
    // Keep it published or update it to Empresa template
    update_post_meta( $old_nosotros->ID, '_wp_page_template', 'page-empresa.php' );
}

$old_sol_medida = get_page_by_path( 'soluciones-a-medida' );
if ( $old_sol_medida ) {
    update_post_meta( $old_sol_medida->ID, '_wp_page_template', 'page-soluciones.php' );
}
...
WP_CLI::success( "Arquitectura de navegación y páginas sincronizada con éxito." );
